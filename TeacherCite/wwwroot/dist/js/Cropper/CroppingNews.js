...
const saveImage = () => { outimage.src = out; }
const addNews = () => {
	if (out && state) {
		state = false;
		...
		$.ajax('/Teacher/News/AddNews', {
			...
			contentType: false,
			success(data) {
				M.toast({ html: '<blockquote class="green-text">News: ' + data + '  added!</blockquote>', classes: 'rounded' });
			...
			error() {
				...
				M.toast({ html: '<blockquote class="red-text">Upload error.</blockquote>', classes: 'rounded' });
				state = true;
			},
		});
	}
